...
```python
# read the data from file. the files with list of track uris
...
# spotify.track('spotify:track:14J3PO0VnhtcRa31r7Aj1L') 
```
...
# There are few columns that wont be required for computation.
# so we are keeping that on the right side of the data frame for easy calculation as those columns can be filtered out later.
mpd_df_side = mpd_df[['artist_popularity','explicit','artist_genres','song_name']]

# dropping file from the main df and later we will add these to the right side
...
# we are standardizing the features to use in our recommendation model
ss = StandardScaler()
...
# scaled data to be used on the list of songs that we want to predict like a track list or one song.
# in case of playlist inputs, this scaler will be used to scale list of songs from the input playlist
